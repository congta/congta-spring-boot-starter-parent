...
/**
 * copied from org.bson.types.ObjectId in org.mongodb:bson:4.2.3
 *
 * <p>A globally unique identifier for objects.</p>
 *
 * <p>Consists of 12 bytes, divided as follows:</p>
 * <table border="1">
 * <caption>SessionId layout</caption>
 * <tr>
...
public final class SessionId implements Comparable<SessionId>, Serializable {
     ...
     * @since 4.1
     */
    public static SessionId getSmallestWithDate(final Date date) {
        return new SessionId(dateToTimestampSeconds(date), 0, (short) 0, 0, false);
    }

    /**
     * Checks if a string could be an {@code SessionId}.
     *
     * @param hexString a potential SessionId as a String.
     ...
     * @param date the date
     */
    public SessionId(final Date date) {
     ...
     */
    public SessionId(final Date date, final int counter) {
        this(dateToTimestampSeconds(date), counter, true);
    }

    /**
     * Creates an SessionId using the given time, machine identifier, process identifier, and counter.
     *
     * @param timestamp the time in seconds
     * @param counter   the counter
     * @throws IllegalArgumentException if the high order byte of counter is not zero
     */
    public SessionId(final int timestamp, final int counter) {
        this(timestamp, counter, true);
    }

    private SessionId(final int timestamp, final int counter, final boolean checkCounter) {
        this(timestamp, RANDOM_VALUE1, RANDOM_VALUE2, counter, checkCounter);
    }

    private SessionId(final int timestamp, final int randomValue1, final short randomValue2, final int counter,
     ...
     * @throws IllegalArgumentException if the string is not a valid hex string representation of an SessionId
     */
    public SessionId(final String hexString) {
     ...
     */
    public SessionId(final byte[] bytes) {
     ...
     * @since 3.4
     */
    public SessionId(final ByteBuffer buffer) {
        ...
        // and SessionId's are always in big-endian order.
     ...
     *
     * @return a string representation of the SessionId in hexadecimal format
     */
    ...
    public String toBase64String() {
        return Base64Utils.encodeToUrlSafeString(toByteArray());
    }
    ...
    public boolean equals(final Object o) {
        ...
        if (counter != sessionId.counter) {
            return false;
        }
        if (timestamp != sessionId.timestamp) {
            return false;
        }

        if (randomValue1 != sessionId.randomValue1) {
            return false;
        }

        if (randomValue2 != sessionId.randomValue2) {
            return false;
        }
    ...
    @Override
    public int compareTo(final SessionId other) {
        if (other == null) {
    ...
    private static class SerializationProxy implements Serializable {
        ...
        SerializationProxy(final SessionId sessionId) {
            bytes = sessionId.toByteArray();
        }
    ...
    private static byte[] parseHexString(final String s) {
        if (!isValid(s)) {
            throw new IllegalArgumentException("invalid hexadecimal representation of an SessionId: [" + s + "]");
        }
